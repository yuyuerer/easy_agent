...
def get_api_keys():
    """Helper function to retrieve API keys from config."""
    api_key = config.GOOGLE_API_KEY
    cse_id = config.GOOGLE_CSE_ID

    if not api_key or not cse_id:
        raise ValueError(
            "Missing required API keys in config. Please set GOOGLE_API_KEY and GOOGLE_CSE_ID in config.py."
        )
